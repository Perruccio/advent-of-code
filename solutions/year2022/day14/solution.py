import aoc.parse
import aoc.math
from aoc.geometry import Point2D
from aoc import aoc
...
def get_input(file):
    ...
    return aoc.parse.map_by_line(aoc.read_input(2022, 14, file), parse_wall)
...
def make_walls(data):
    ...
    for wall in data:
        ...
        for edge1, edge2 in zip(wall, wall[1:]):
            diff = edge2 - edge1
            dir = aoc.math.sign(diff)
            rocks.update(edge1 + dir * step for step in range(1 + aoc.math.norm(diff, "inf")))
    return rocks
